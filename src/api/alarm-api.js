...
function clearAlarm(vm, alarmId, callback) {
  const params = new URLSearchParams();
  params.append('id', alarmId);
  // const data = { id: alarmId }
  // const params = qs.stringify(data)

  http.simplePost(
    vm,
    constants.URL_ALARM_CLEAR,
    params,
    { 'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8' },
    (response) => {
      // 处理成功情况
      http.notifySuccess(vm, `告警 [${alarmId}] Clear操作成功`)
      if (callback) {
        callback(response)
      }
    }
...
function createAlarms(vm, alarmArray, callback) {
  http.simplePost(vm, constants.URL_ALARM_CREATE, alarmArray, null, (response) => {
    // 处理成功情况
...
function updateAlarm(vm, alarm, callback) {
  http.simplePost(vm, constants.URL_ALARM_UPDATE, alarm, null, (response) => {
    // 处理成功情况
...
function getAlarm(vm, alarmId, callback) {
  ...
  http.simpleGet(vm, reqUrl, null, (response) => {
    // 处理成功情况
